// ============================================================
// PERLLON — Admin panel (hash-routed SPA)
// ============================================================
// Routes: #/login, #/dashboard, #/products, #/products/new,
//         #/products/:id, #/spotlight, #/hero
...
import { uploadMedia, deleteMedia, mediaPublicUrl, HERO_BUCKET } from '../lib/storage.js';
...
import { renderLogin, sessionUnavailable, accessDenied, notConfigured } from './auth-view.js';
...
const productFormContext = { root, shell, isCurrentNavigation, router, imageUrlOrNull };

const routes = {
  login: renderLogin,
  dashboard: renderDashboard,
  products: renderProducts,
  'products/new': (id, runId) => renderProductForm(id, runId, productFormContext),
  'products/:id': (id, runId) => renderProductForm(id, runId, productFormContext),
...
async function renderProducts(_id, runId) {
  ...
  try {
    ...
    const rows = products.map((p) => {
      ...
      return `
        <tr>
          <td>${img ? `<img class="thumb" src="${esc(img)}" alt="" loading="lazy" decoding="async">` : '<span style="color:var(--muted)">—</span>'}</td>
    ...
    const applyFilter = () => {
      ...
      $('#products-tbody').innerHTML = filtered.length
        ? filtered.map((p) => {
            ...
            return `<tr>
              <td>${img ? `<img class="thumb" src="${esc(img)}" alt="" loading="lazy" decoding="async">` : '—'}</td>
              <td><a href="#/products/${p.id}" style="color:var(--navy);font-weight:600;text-decoration:none">${esc(p.name)}</a></td>
